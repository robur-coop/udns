...
module Name = struct
  ...
  let encode : ?compress:bool -> 'a Domain_name.t -> int Domain_name.Map.t ->
    ...
    let name = Domain_name.domain name in
...
module Soa = struct
  type t = {
    nameserver : [ `host ] Domain_name.t ;
    hostmaster : [ `domain ] Domain_name.t ;
    serial : int32 ;
    refresh : int32 ;
    retry : int32 ;
    expiry : int32 ;
  ...
  let decode names buf ~off ~len:_ =
    ...
    Name.host off nameserver >>= fun nameserver ->
...
module Rr_map = struct
  ...
  let encode : type a. ?clas:Class.t -> [ `domain ] Domain_name.t -> a key -> a -> Name.name_offset_map -> Cstruct.t -> int ->
...
module Name_rr_map = struct
  ...
  let find : type a . [ `domain ] Domain_name.t -> a Rr_map.rr -> t -> a option =
    fun name k dmap ->
...
module Packet = struct
  ...
  module Question = struct
    ...
    type t = [ `domain ] Domain_name.t * [ qtype | `Axfr | `Ixfr ]
    ...
    let create : type a b. b Domain_name.t -> a Rr_map.key -> t =
      fun name k -> Domain_name.domain name, `K (K k)
          ...
          let (names, off), count, alias =
            Rr_map.fold (fun (Rr_map.B (k, v)) ((names, off), count, alias) ->
                let alias' = match k, v with
                  | Cname, (_, alias) -> Some (Domain_name.domain alias)
                  | _ -> alias
                in
                let r, amount = Rr_map.encode name k v names buf off in
  ...
  module Ixfr = struct
    ...
    let rec rrs_and_soa buf names off count acc =
      ...
      | n ->
        ...
        match k, v with
        | Rr_map.Soa, soa ->
          Ok (acc, (Some (name, soa) : ([ `domain ] Domain_name.t * Soa.t) option), pred n, names, off)
        | _ ->
  ...
  type t = {
    ...
    edns : Edns.t option ;
    tsig : ([ `domain ] Domain_name.t * Tsig.t * int) option ;
  }
...
module Tsig_op = struct
  type e = [
    | `Bad_key of [ `domain ] Domain_name.t * Tsig.t
    | `Bad_timestamp of [ `domain ] Domain_name.t * Tsig.t * Dnskey.t
    | `Bad_truncation of [ `domain ] Domain_name.t * Tsig.t
    | `Invalid_mac of [ `domain ] Domain_name.t * Tsig.t
  ]
  ...
  type verify = ?mac:Cstruct.t -> Ptime.t -> Packet.t ->
    [ `domain ] Domain_name.t -> ?key:Dnskey.t -> Tsig.t -> Cstruct.t ->
  ...
  type sign = ?mac:Cstruct.t -> ?max_size:int -> [ `domain ] Domain_name.t ->
