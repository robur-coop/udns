...
module Dnskey : sig
  ...
  val name_key_of_string : string -> ([ `domain ] Domain_name.t * t, [> `Msg of string ]) result
...
module Tsig : sig
  ...
  val encode_raw : [ `domain ] Domain_name.t -> t -> Cstruct.t

  val encode_full : [ `domain ] Domain_name.t -> t -> Cstruct.t
...
module Name_rr_map : sig
  ...
  val add : [ `domain ] Domain_name.t -> 'a Rr_map.key -> 'a -> t -> t

  val find : [ `domain ] Domain_name.t -> 'a Rr_map.key -> t -> 'a option

  val remove_sub : t -> t -> t

  val singleton : [ `domain ] Domain_name.t -> 'a Rr_map.key -> 'a -> t
