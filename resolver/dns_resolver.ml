...
type key = [ `domain ] Domain_name.t * Packet.Question.qtype
...
type awaiting =
  int64 * int * proto * [ `domain ] Domain_name.t * Edns.t option * Ipaddr.V4.t * int * key * int
    ...
    | Ok (`Entry Rr_map.(B (Ns, (_, names))), _) ->
      let ips =
        Domain_name.Host_set.fold (fun name acc ->
            match Dns_resolver_cache.cached t.cache now A (Domain_name.domain name) with
