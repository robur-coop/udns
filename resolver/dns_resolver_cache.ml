...
module V = struct
  ...
  type t =
    | Alias of meta * int32 * [ `domain ] Domain_name.t
    | No_domain of meta * [ `domain ] Domain_name.t * Soa.t
...
let find_nearest_ns rng ts t name =
  ...
  let rec go nam =
    ...
    | None -> or_root go nam
    | Some ns ->
      let host = Domain_name.domain ns in
...
let resolve t ~rng ts name typ =
  ...
  let rec go t typ name =
    ...
    match find_nearest_ns rng ts t (Domain_name.domain name) with
