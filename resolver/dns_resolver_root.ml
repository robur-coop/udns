...
let a_records =
  List.map (fun (name, ip) ->
      Domain_name.domain name, Rr_map.(B (A, (a_ttl, Ipv4_set.singleton ip))))
    root_servers
...
let stub_soa s =
  let nameserver = Domain_name.(host_exn (prepend_label_exn s "ns"))
...
let root_servers = List.map (fun (n, ip) -> Domain_name.domain n, ip) root_servers
