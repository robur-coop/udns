...
let namekey_c =
  let parse s =
    ...
    | Some (name, key) -> match Domain_name.of_string ~hostname:false name, Udns_types.dnskey_of_string key with
  ...
  parse, fun ppf (name, zone, key) -> Fmt.pf ppf "key name %a zone %a dnskey %a"
      Domain_name.pp name Domain_name.pp zone Udns_types.pp_dnskey key
