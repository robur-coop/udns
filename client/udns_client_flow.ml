...
struct
  ...
  let getaddrinfo (type requested) t ?nameserver (query_type:requested Udns.Umap.k) name
  ...
  let gethostbyname stack ?nameserver domain =
    ...
    getaddrinfo stack ?nameserver Udns.Umap.A domain >>= fun (_ttl, resp) ->
    match Udns.Umap.Ipv4_set.choose_opt resp with
