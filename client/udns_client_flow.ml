...
struct
  ...
  let getaddrinfo (type requested) t ?nameserver (query_type:requested Udns.Map.k) name
  ...
  let gethostbyname stack ?nameserver domain =
    ...
    getaddrinfo stack ?nameserver Udns.Map.A domain >>= fun (_ttl, resp) ->
    match Udns.Map.Ipv4_set.choose_opt resp with
