...
module Trie = struct
  ...
  let l_ok =
    let module M = struct
      type t = Rr_map.b * ([ `domain ] Domain_name.t * int32 * Domain_name.Host_set.t)
  ...
  let simple () =
    ...
    let soa = {
      Soa.nameserver = Domain_name.host_exn (n_of_s "a") ; hostmaster = n_of_s "hs" ;
  ...
  let basic () =
    let soa = {
      Soa.nameserver = Domain_name.host_exn (n_of_s "ns1.foo.com") ;
