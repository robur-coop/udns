...
let follow_res =
  let module M = struct
    type t =
      ...
      | `Query of [ `domain ] Domain_name.t * Dns_resolver_cache.t
      ]
      let pp ppf = function
...
let res =
  let module M = struct
    type t = (Rr_map.k * [ `domain ] Domain_name.t * Dns_resolver_cache.rank * Dns_resolver_cache.res) list
    let pp ppf xs =
...
let scrub_soa () =
  ...
  let q = q_name, `K (Rr_map.K A) in
  let soa = {
    Soa.nameserver = Domain_name.host_exn (name "a") ; hostmaster = name "b" ;
...
let scrub_rfc2308_2_1 () =
  ...
  let q = q_name, `K (Rr_map.K A) in
  let soa = {
    Soa.nameserver = Domain_name.host_exn (name "ns1.xx") ; hostmaster = name "hostmaster.ns1.xx" ;
